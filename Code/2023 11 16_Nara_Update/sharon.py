...
class imageProcessing():
    ...
    def color_thresh(self,file):
        ...
        # print(coefficients)
        return x_fit,y,perspective_transform,coefficients
...
class Motion(object):
    def __init__(self):
        ...
        # nDrive
        # self.coeff=self.img_processing.coeff
    ...
    def cycle(self):
        ...
        #for i in images:
            ...
            print('i',i)
            i=i+1
            xx=self.img_processing.get_x()
            yy=self.img_processing.get_y()
            output_img=self.img_processing.get_binary_image()
            # self.coeff = self.img_processing.coeff
            # self.coeff=self.img_processing.get_coeff()
            coeff = self.img_processing.coeff
            print("coeff",coeff)
            err1,err2,xx,yy=self.error_compute(340,coeff,'dist')
            ...
            cv2.circle(output_img, (xx, yy), 10, (0, 0, 0), -1)

            #cv2.imshow('hough', output_image_path)
            cv2.imshow('hsv_perspective', output_img)
            #cv2.waitKey(0)
