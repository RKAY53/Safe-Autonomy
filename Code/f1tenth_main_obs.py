...
class lidarOperation():
    def __init__(self):
        lidar_topic = "/scan"
        rospy.Subscriber(lidar_topic, LaserScan, self.lidar_callback)
        self.lidarReading=None
                       
    def lidar_callback(self,msg):
        self.lidarReading=msg.ranges
        #print(msg.ranges)
    def obstacleDetection(self):
        #-50,50
        idx_1=85*4
        idx_2=185*4
        lidar_front100=self.lidarReading[idx_1:idx_2]
        stop_dist=1
        avoid_dist=2
        avoid_signal=False
        stop_signal=False
        if np.min(lidar_front100)<stop_dist:
            stop_signal=True
        if np.min(lidar_front100)<avoid_dist:
            avoid_signal=True
        return stop_signal,avoid_signal

    #new
    def obstacleAvoidance(self,dir_a):
        #-90 to 90
        idx_1=45*4
        idx_2=225*4
        lidar_front_side=self.lidarReading[idx_1:idx_2]
        space={}
        #left_space={}
        #need 20 degree
        min_deg=20
        ...
        for i in range(15*4,len(lidar_front_side)-15*4):
            if lidar_front_side[i]<1.25:
                if count_n!=0:
                    if count_n>=min_deg*4:
                        space[(s_pt,i-1)]=count_n
                count_n=0
            else:
                count_n+=1
                if count_n==1:
                    s_pt=i
        '''
                    for i in range(int(len(lidar_front_side)/2)+15*4,len(lidar_front_side)-15*4):
            if lidar_front_side[i]<1.25:
                if count_n!=0:
                    if count_n>=min_deg*4:
                        right_space[(s_pt,i-1)]=count_n
                count_n=0
            else:
                count_n+=1
                if count_n==1:
        ...
        if dir_a is None:
            if len(space) != 0:
                max_key = max(space, key=space.get)
        elif dir_a=='r':
            if len(space) != 0:
                max_key=list(space.keys())[0]
        elif dir_a=='l':
            if len(space) != 0:
                max_key=list(space.keys())[-1]

        if max_key is None:
            return signal,0
        signal=True
        target=int(((max_key[0]+max_key[1])/2-90*4)/4)
...
class Motion(object):
    def __init__(self):
        ...
        #self.drive_msg.drive.speed     = 0.8 #, reference speed 1.0
        self.lidar=lidarOperation()
    ...
    def cycle(self):
        ...
        #for i in images:
            ...
            #print('i',i)
            i=i+1
            possible_signal=True
            obs_found,avoid_signal=self.lidar.obstacleDetection()
            #signal=True found sthing
            if avoid_signal:
                possible_signal,steering_avoid=self.lidar.obstacleAvoidance(dir)
            #signal =>can avoid?
            if obs_found is True or possible_signal is False:
                print('obstacle found')
                target_velocity= 0
            #print('output_img',self.raw_img)
            elif avoid_signal is True and possible_signal is True:
                avoiding=True
                print('avoidance') 
                target_steering=steering_avoid
                self.prev_steering=target_steering
                steering_list.append[target_steering]
                if steering_avoid>0:
                    dir='r'
                elif steering_avoid<0:
                    dir='l'
            elif self.raw_img is not None:
                print('------------------')
                #print('output_img',self.raw_img)
                ...
                if curr_found_line is True:
                    ...
                    err1,err2,xx,yy=self.error_compute(400,self.coeff,'dist')
                    err.append([i,err1])
                    ...
                    if avoiding is True:
                        if len(steering_list)==0:
                            target_steering=-steering_list.pop()
                        else:
                            avoiding=False
